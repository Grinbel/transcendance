...
export const interceptor_response = axiosInstance.interceptors.response.use(
    ...
    error => {
      ...
      if (status)
        {
          //log the _retry flag of originalRequest
          if (status === 401) 
          {
            console.log('we got first error now we will try to refresh token')
              ...
              return axiosInstance
                ...
                .then((response) => {
                  ...
                  if (response.status === 200)
                  {
                    ...
                    onRrefreshing(response.data.access);
                    return axiosInstance(originalRequest);
                  }
                })
                .catch(err => {
                  // if the refresh token is expired we will redirect the user to the login page
                  console.log('we got an error in axios, the refresh token is dead too user have to login again: ', err.response.status);
                  console.log('error message', err.response.config.data);
          ...
          } else if (status === 401 && originalRequest._retry === true) {
          // if the refresh token is expired we will redirect the user to the login page
