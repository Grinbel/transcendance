...
function MultiGame() {
	const { t } = useTranslation();
		...
		let player_names = ['Yoshi', 'Peach', 'Bob', 'Bowser' , 'Mario', 'Benoit le BG']
						...
						if(options.nb_buttons === 1)
							{
								...
								if(options.nb_players > 2)
									message += player_names[2] + " : C <br>"
								if(options.nb_players > 3)
									message += player_names[3] + " : 1 <br>"
								if(options.nb_players > 4)
									message += player_names[4] + " : U <br>"
								if(options.nb_players > 5)
								message += player_names[5] + " : > <br>"
							}
						else
						{
		...
		function show_rules()
		{
			...
			message += t('multi_to_get') + options.score_to_get + t('multi_to_get2') + options.score_diff + t('multi_to_get3') + "<br>"
			return message;
		}
						...
						message.innerHTML += t('goal_1') + options.score_to_get + t('goal_2') + options.score_diff + t('goal_3')
	...
	//	controls = new OrbitControls( camera, renderer.domElement );
		...
		function ball_reset() {
			...
			if (hitters)
			{
				...
				if ((sortedScores[0] - sortedScores[1] >= options.score_diff) && sortedScores[0] >= options.score_to_get)
				{
					options.ball_pause = -1
					scene.clear();
                    for(let i = 0; i<options.nb_players; i++)
